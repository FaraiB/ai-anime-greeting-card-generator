...
const LoadingSpinner: React.FC = () => (
    <div className="flex flex-col items-center justify-center h-96 text-pink-500">
        <svg className="animate-spin -ml-1 mr-3 h-12 w-12 text-pink-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
...
const ErrorDisplay: React.FC<{ message: string }> = ({ message }) => (
    <div className="w-full max-w-sm h-96 bg-red-50 border-2 border-red-200 rounded-2xl flex flex-col items-center justify-center p-4 text-center">
        <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-red-400 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
        <h3 className="text-lg font-bold text-red-600">Oops! Something went wrong.</h3>
...
const App: React.FC = () => {
    const [selectedOccasionId, setSelectedOccasionId] = useState<string>(OCCASIONS[0].id);
    ...
    const handleGenerateCard = useCallback(async () => {
        ...
        try {
            ...
            setError(err.message || "An unknown error occurred. Please check the console or try again.");
        } finally {
            setIsLoading(false);
        }
    }, [selectedOccasion]);
                    ...
                    <div className="bg-white/60 backdrop-blur-sm p-6 sm:p-8 rounded-2xl shadow-lg w-full max-w-lg mb-8">
                        ...
                        />
                         <div className="text-center mt-6">
                            <h2 className="text-xl font-semibold text-center text-gray-700 mb-4">2. Create Your Card!</h2>
                            <button
